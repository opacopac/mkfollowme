...
public class OsmMapPainter
{
	...
	public static void drawScale(Graphics g, OsmPosition mapCoordinates)
	{
		...
		double maxMeter = mapCoordinates.getMeterDistX(g.getClipWidth() / 2);
		...
		g.setColor(0x000000);
		g.drawLine(5, g.getClipHeight() - 5, 5 + scalePixel, g.getClipHeight() - 5);
		g.drawLine(5, g.getClipHeight() - 5, 5, g.getClipHeight() - 7);
		g.drawLine(5 + scalePixel, g.getClipHeight() - 5, 5 + scalePixel, g.getClipHeight() - 7);
		g.drawString(text, 7, g.getClipHeight() - 7, Graphics.BOTTOM | Graphics.LEFT);
	}
	...
	private static void drawTile(Graphics g, OsmPosition mapCoordinates, OsmPosition tilePos)
	{
        ...
        {        
	        Image img = OsmTileLoader.getInstance().getMapImage(tilePos);
	        if (img != null)
	        	g.drawImage(img, screenX, screenY, Graphics.TOP | Graphics.LEFT);
        }
	}
	...
	private static boolean isOutOfScreen(Graphics g, long x, long y, int tolerance)
	{
		...
		if (x > g.getClipWidth() + tolerance)
			return true;
		
		if (y > g.getClipHeight() + tolerance)
			return true;
	...
	private static boolean isTileOutOfScreen(Graphics g, long x, long y, int tolerance)
	{
		...
		long intersectX2 = Math.min(x + OsmPosition.TILE_PIXEL_WIDTH, g.getClipWidth() + tolerance);
		long intersectY = Math.max(y, -tolerance);
		long intersectY2 = Math.min(y + OsmPosition.TILE_PIXEL_HEIGHT, g.getClipHeight() + tolerance);
	...
	{
		double propX = g.getClipWidth() / (double)OsmPosition.TILE_PIXEL_WIDTH;
		double propY = g.getClipHeight() / (double)OsmPosition.TILE_PIXEL_HEIGHT;
