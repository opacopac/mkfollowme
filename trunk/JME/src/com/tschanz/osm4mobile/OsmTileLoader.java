...
//	private static final int MinTileStoreFreeBytes = 50000;
    ...
    public Image getMapImage(OsmPosition position)
    {
    	...
    	if (this.memCache.containsKey(url))
    		return (Image)this.memCache.get(url);
    	
    	// 2nd try local store
    	if (this.recordLookupTable.contains(url))
    	{
    		Image img = this.loadFromStore(url);
    		if (img != null)
    			return img;
    		else
	...
	private Image loadFromStore(String url)
	{
...
//		}
	...
	private void putToMemCache(String url, Image img)
	{
    ...
    private Image getEmptyTile()
    {
    	Image img = Image.createImage(OsmPosition.TILE_PIXEL_WIDTH, OsmPosition.TILE_PIXEL_HEIGHT);
